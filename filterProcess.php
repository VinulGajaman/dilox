...
if ($f == "0") {
    $category = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1';");

    $d = $category->num_rows;
    $results_per_page = 5;
    $number_of_pages = ceil($d / $results_per_page);
    $offset = ((int)$pageno - 1) * $results_per_page;
    $selectedproducts = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1' LIMIT $results_per_page OFFSET $offset");
    $spn = $selectedproducts->num_rows;
} else if ($f == "1") {

    $category = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1' ORDER BY `datetime_added` DESC;");
    $d = $category->num_rows;
    $results_per_page = 5;
    $number_of_pages = ceil($d / $results_per_page);
    $offset = ((int)$pageno - 1) * $results_per_page;
    $selectedproducts = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1' ORDER BY `datetime_added` DESC LIMIT $results_per_page OFFSET $offset");
    $spn = $selectedproducts->num_rows;
} else if ($f == "2") {
    $category = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1' ORDER BY `price` ASC;");
    $d = $category->num_rows;
    $results_per_page = 5;
    $number_of_pages = ceil($d / $results_per_page);
    $offset = ((int)$pageno - 1) * $results_per_page;
    $selectedproducts = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1' ORDER BY `price` ASC LIMIT $results_per_page OFFSET $offset");
    $spn = $selectedproducts->num_rows;
} else if ($f == "3") {
    $category = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1' ORDER BY `price` DESC;");
    $d = $category->num_rows;
    $results_per_page = 5;
    $number_of_pages = ceil($d / $results_per_page);
    $offset = ((int)$pageno - 1) * $results_per_page;
    $selectedproducts = Database::search("SELECT * FROM `product` WHERE `category_id`='" . $cid . "' AND `status_id`='1' ORDER BY `price` DESC LIMIT $results_per_page OFFSET $offset");
    $spn = $selectedproducts->num_rows;
}
...
<div class="col-12 mb-3 mt-3">
    <div class="pagination d-flex justify-content-center">
        ...
        if ($pageno != 1) {
        ?>
            <button class=" btn btn-outline-dark" onclick="filter(<?php echo $f . ' ,' . $cid . ' ,' . $pageno - 1; ?>);">&laquo;</button>
            <?php
        }
        ...
        for ($page = 1; $page <= $number_of_pages; $page++) {
            if ($page == $pageno) {
            ?>
                <button class="ms-1 btn btn-outline-dark active" onclick="filter(<?php echo $f . ' ,' . $cid . ' ,' . $page; ?>);"><?php echo $page; ?></button>
            <?php
            } else {
            ?>
                <button class="ms-1 btn btn-outline-dark" onclick="filter(<?php echo $f . ' ,' . $cid . ' ,' . $page; ?>);"><?php echo $page; ?></button>
        <?php
            }
        }
        ?>
        ...
        ?>
            <button class="ms-1 btn btn-outline-dark" onclick="filter(<?php echo $f . ' ,' . $cid . ' ,' . $pageno + 1; ?>);">&raquo;</button>
        <?php
        }
        ?>
    </div>
